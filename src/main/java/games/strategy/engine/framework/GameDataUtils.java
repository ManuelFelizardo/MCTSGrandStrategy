...
public final class GameDataUtils {
  ...
  public static GameData cloneGameDataWithoutHistoryFast(
      final GameData data, final boolean copyDelegates) {
    final History temp = data.getHistory();
    data.resetHistory();
    final GameData dataCopy = cloneGameDataFast(data);
    data.setHistory(temp);
  ...
  public static GameData cloneGameDataFast(final GameData data) {
    try {
      final byte[] bytes =
          IoUtils.writeToMemory(os -> GameDataManager.saveGameFast(os, data));
      return IoUtils.readFromMemory(bytes, GameDataManager::loadGameFast);
    } catch (final IOException e) {
      log.log(Level.SEVERE, "Failed to clone game data", e);
      return null;
    }
